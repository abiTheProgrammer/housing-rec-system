...
const insertValuesIntoTable = (db, formData, callback) => {
  ...
  db.query(selectQuery, (selectError, results) => {
    if (selectError) {
      console.error(
        "Error executing SELECT query frcom user_data table:",
        selectError
      );
      return;
    }
...
const calculateRiskScore = (db, formData) => {
  ...
  db.query(selectQuery, (selectError, results) => {
    if (selectError) {
      console.error(
        "Error executing SELECT query from user_data to calculate user_risk_score:",
        selectError
      );
      return;
    }
    ...
    db.query(insertQuery, (insertError) => {
      if (insertError) {
        console.error(
          "Error executing INSERT query into user_risk_score table:",
          insertError
        );
      } else {
      ...
      db.end();
      console.log(
        "Database connection is closed after calculating risk score."
      );
    });
  });
};
...
app.put("/", (req, res) => {
  ...
  insertValuesIntoTable(db, formData, (isDisconnected) => {
    ...
    if (!isDisconnected) {
      ...
      console.log(
        "Database connection is closed. Cannot calculate risk score."
      );
    }
  });
});
